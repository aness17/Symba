...
class Actual_model extends CI_Model
{
    ...
    public function summarycapex($thn)
    {
        // return $this->db->query('SELECT tdetail_budget.category_budget,tdetail_budget.currency bd,tactual.currency cr,tdivision.division,tstation.code_station,tdetail_budget.id_user, 
        ...
        // FROM `tactual` RIGHT JOIN `tdetail_budget` on tdetail_budget.id_budget = tactual.id_budget 
        // JOIN (SELECT tdetail_budget.category_budget,tdetail_budget.currency,tdetail_budget.id_user, SUM(tdetail_budget.amount_debit) debit_budget,
        // SUM(tdetail_budget.amount_credit)credit_budget FROM tdetail_budget JOIN tuser ON tuser.id_user = tdetail_budget.id_user 
        // WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "CAPEX" GROUP BY tdetail_budget.id_user) tbl_budget_user 
        // ON tbl_budget_user.id_user = tdetail_budget.id_user JOIN `tuser` ON tuser.id_user = tdetail_budget.id_user 
        // JOIN `tdivision` on tdivision.id_dvn = tuser.id_dvn JOIN `tstation` ON tstation.id_station = tdivision.id_station 
        // WHERE tdetail_budget.budget_year = ' . $thn . ' OR tbl_budget_user.category_budget = "CAPEX" GROUP BY tdetail_budget.id_user ASC 
        // ORDER BY tstation.code_station ASC;')->result_array();
        ...
        // GROUP BY tdetail_budget.id_user, tdetail_budget.category_budget ASC ORDER BY tstation.code_station asc;')->result_array();

        return $this->db->query('SELECT tdetail_budget.category_budget,tdetail_budget.currency bd,tactual.currency cr,tdivision.division,tstation.code_station,tdetail_budget.id_user, 
        tdetail_budget.id_budget,debit_budget,credit_budget,SUM(tactual.amount_debit) debit_actual, SUM(tactual.amount_credit) credit_actual FROM `tactual` 
        RIGHT JOIN `tdetail_budget` on tdetail_budget.id_budget = tactual.id_budget 
        LEFT JOIN (SELECT tdetail_budget.category_budget,tdetail_budget.currency,tdetail_budget.id_user, 
        SUM(tdetail_budget.amount_debit) debit_budget,SUM(tdetail_budget.amount_credit)credit_budget FROM tdetail_budget JOIN tuser 
        ON tuser.id_user = tdetail_budget.id_user WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "CAPEX" GROUP BY tdetail_budget.id_user, tdetail_budget.category_budget) tbl_budget_user 
        ON tbl_budget_user.id_user = tdetail_budget.id_user JOIN `tuser` ON tuser.id_user = tdetail_budget.id_user JOIN `tdivision` 
        on tdivision.id_dvn = tuser.id_dvn JOIN `tstation` ON tstation.id_station = tdivision.id_station WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "CAPEX"
    ...
    public function summary($thn)
    {
        ...
        JOIN (SELECT tdetail_budget.category_budget,tdetail_budget.currency,tdetail_budget.id_user, SUM(tdetail_budget.amount_debit) debit_budget,
        SUM(tdetail_budget.amount_credit)credit_budget FROM tdetail_budget JOIN tuser ON tuser.id_user = tdetail_budget.id_user 
        WHERE tdetail_budget.budget_year = ' . $thn . ' GROUP BY tdetail_budget.id_user) tbl_budget_user 
        ON tbl_budget_user.id_user = tdetail_budget.id_user JOIN `tuser` ON tuser.id_user = tdetail_budget.id_user 
        JOIN `tdivision` on tdivision.id_dvn = tuser.id_dvn JOIN `tstation` ON tstation.id_station = tdivision.id_station 
        WHERE tdetail_budget.budget_year = ' . $thn . ' GROUP BY tdetail_budget.id_user ASC 
        ORDER BY tstation.code_station ASC;')->result_array();
        // return $this->db->query('select tdivision.division,sum(tdetail_budget.amount_debit) debitbgt, sum(a.amount_debit) as debit ,SUM(a.amount_credit) as credit,sum(tdetail_budget.amount_debit)debitbgt from tactual a RIGHT join tdetail_budget on a.id_budget = tdetail_budget.id_budget join tuser ON tuser.id_user = tdetail_budget.id_user join tdivision on tdivision.id_dvn = tuser.id_dvn GROUP BY tuser.id_user')->result_array();
    ...
    public function summaryopex($thn)
    {
        // return $this->db->query('SELECT tdetail_budget.category_budget,tdetail_budget.currency bd,tactual.currency cr,tdivision.division,tstation.code_station,tdetail_budget.id_user, 
        // tdetail_budget.id_budget,debit_budget,credit_budget,SUM(tactual.amount_debit) debit_actual, SUM(tactual.amount_credit) credit_actual 
        // FROM `tactual` RIGHT JOIN `tdetail_budget` on tdetail_budget.id_budget = tactual.id_budget 
        // JOIN (SELECT tdetail_budget.category_budget,tdetail_budget.currency,tdetail_budget.id_user, SUM(tdetail_budget.amount_debit) debit_budget,
        // SUM(tdetail_budget.amount_credit)credit_budget FROM tdetail_budget JOIN tuser ON tuser.id_user = tdetail_budget.id_user 
        // WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "OPEX" GROUP BY tdetail_budget.id_user) tbl_budget_user 
        // ON tbl_budget_user.id_user = tdetail_budget.id_user JOIN `tuser` ON tuser.id_user = tdetail_budget.id_user 
        // JOIN `tdivision` on tdivision.id_dvn = tuser.id_dvn JOIN `tstation` ON tstation.id_station = tdivision.id_station 
        // WHERE tdetail_budget.budget_year = ' . $thn . ' OR tbl_budget_user.category_budget = "OPEX" GROUP BY tdetail_budget.id_user ASC 
        // ORDER BY tstation.code_station ASC;')->result_array();
        return $this->db->query('SELECT tdetail_budget.category_budget,tdetail_budget.currency bd,tactual.currency cr,tdivision.division,tstation.code_station,tdetail_budget.id_user, 
        tdetail_budget.id_budget,debit_budget,credit_budget,SUM(tactual.amount_debit) debit_actual, SUM(tactual.amount_credit) credit_actual FROM `tactual` 
        RIGHT JOIN `tdetail_budget` on tdetail_budget.id_budget = tactual.id_budget 
        LEFT JOIN (SELECT tdetail_budget.category_budget,tdetail_budget.currency,tdetail_budget.id_user, 
        SUM(tdetail_budget.amount_debit) debit_budget,SUM(tdetail_budget.amount_credit)credit_budget FROM tdetail_budget JOIN tuser 
        ON tuser.id_user = tdetail_budget.id_user WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "OPEX" 
        GROUP BY tdetail_budget.id_user, tdetail_budget.category_budget) tbl_budget_user 
        ON tbl_budget_user.id_user = tdetail_budget.id_user JOIN `tuser` ON tuser.id_user = tdetail_budget.id_user JOIN `tdivision` 
        on tdivision.id_dvn = tuser.id_dvn JOIN `tstation` ON tstation.id_station = tdivision.id_station WHERE tdetail_budget.budget_year = ' . $thn . ' AND tdetail_budget.category_budget = "OPEX"
        GROUP BY tdetail_budget.id_user, tdetail_budget.category_budget ASC  
        ORDER BY `tdetail_budget`.`id_user` ASC;')->result_array();
    }
    ...
    public function getbybudget($id, $tahun, $category)
    {
        ...
        $this->db->where('E.category_budget', $category);
        $this->db->where('A.actual_date like', '%' . date('Y') . '%');
        return $this->db->get($this->table . " as A")->result_array();
    }
